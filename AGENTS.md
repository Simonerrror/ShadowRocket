...
- Это политика данного репозитория. Не утверждайте, что разработчик приложения HAPP прекратил его поддержку.
...
- `shadowrocket_custom.conf`, `shadowrocket_whitelist.conf`, `modules/anti_advertising_custom.module`, `modules/wechat_direct.module`, `rules/adobe_telemetry_custom.list` и GFN/NVIDIA-исключения по умолчанию считаются `custom-only`.
