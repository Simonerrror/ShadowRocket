...
- `sr_wl_tests.conf` — отдельный `custom-only` профиль для проверки пользовательских сценариев и новых routing-гипотез; он не является source of truth для основного профиля.
...
## Ownership файлов
- Редактируются вручную: `shadowrocket.conf`, `shadowrocket_custom.conf`, `shadowrocket_whitelist.conf`, `sr_wl_tests.conf`, `distillate/manifest.json`, `distillate/overlays/*`, `distillate/filters/*`, `rules/adobe_telemetry_custom.list`, `rules/russia_extended.list`, `rules/voice_ports.list`, `modules/GFN-AM.module`.
...
- `shadowrocket_custom.conf`, `shadowrocket_whitelist.conf`, `sr_wl_tests.conf`, `modules/anti_advertising_custom.module`, `rules/adobe_telemetry_custom.list` и GFN/NVIDIA-исключения по умолчанию считаются `custom-only`.
...
- В CI compiled `.dat` можно собирать через `python3 scripts/build_distillate.py --allow-stale-compiled`, чтобы при недоступном compiler-upstream сохранить старый `distillate/dat` и продолжить обновление текстовых артефактов.
...
## Тесты/проверки
- Автоматические тесты минимальные: они должны ловить форматные ошибки, невалидные строки списков, сломанные generated-ссылки и недетерминированную сборку. Поведенческую корректность обхода блокировок фиксируйте через ручные тест-профили и заметки к проверке, а не через хрупкие unit-тесты на внешний интернет.
- Перед build-check запускайте:
