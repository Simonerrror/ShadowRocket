...
```
В `shadowrocket_custom.conf` используется `custom-only` схема `FALLBACK-WL`: приоритет `Poland/Finland` → `Germany/Netherlands` → `other WL`, а внутри каждой корзины выбирается лучший узел по пингу через `url-test`.
...
- **MANUAL-PROXY** — ручной выбор всех VLESS-узлов, включая RU/BY/UA.
- **GOOGLE** — ручной выбор из отфильтрованного списка для Google/Gemini/YouTube (NL VLESS + UAE VLESS + узлы, где одновременно есть `WL` и `VLESS`).
