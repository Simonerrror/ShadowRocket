...
### [Proxy Group]
- **MANUAL-PROXY** — ручной выбор всех узлов из подписки без локального country/protocol-фильтра; `policy-regex-filter=WL` наполняет группу узлами фильтрованной подписки.
...
- **GOOGLE** — отдельная `select`-группа для ручного выбора узла под Google/Gemini/YouTube; `policy-regex-filter=WL` наполняет группу узлами фильтрованной подписки.
