...
- вручную редактируются `shadowrocket.conf`, `shadowrocket_custom.conf`, `shadowrocket_whitelist.conf`, `sr_wl_tests.conf`, `distillate/manifest.json`, `distillate/overlays/*`, `distillate/filters/*`, `rules/adobe_telemetry_custom.list`, `rules/russia_extended.list`, `rules/voice_ports.list`, `modules/GFN-AM.module`, `modules/anti_advertising_custom.header`, `modules/instagram-meta.module`, `modules/instagram-meta-full-fix.sgmodule`;
...
Политика изменений:
- `shadowrocket_custom.conf`, `shadowrocket_whitelist.conf`, `sr_wl_tests.conf` и `modules/anti_advertising_custom.module` считаются `custom-only` и содержат single-user/GFN/test-profile логику.
